<?php
class WeiXinQzgdwlAction extends Action {
	...
	private function accessTokenQzgdwl() {
		$tokenFile = "access_token_qzgdwl.txt";//缓存文件名
		$data = json_decode(file_get_contents($tokenFile));
		//print_r($data);
		if ($data->expire_time < time() or !$data->expire_time) {
		$appid = "wxb7d92901cbc8513a";
		$appsecret = "9ba214895fd6d196851c4a448fbd4229";
		$url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=$appid&secret=$appsecret";
		  $res = $this->getJson($url);
		  $access_token = $res['access_token'];
		  if($access_token) {
	...
	public function updateGroup(){		
		$access_token = $this->accessTokenQzgdwl();
		$user = M('User');
		$userRow = $user->where(array('createdate'=>array('egt','2017-05-01 00:00:00'),'is_binding'=>1,'openid_new'=>array('EXP','IS NOT NULL')))->select();
		$openid_list = "";
		$num = 1;
		foreach($userRow as $userRow1){
			$openid_list = $openid_list."\"".$userRow1['openid_new']."\"".",";
			$num++;
			if($num%50 == 0){
				$openid_list = substr($openid_list,0,-1);
				$data = "{\"openid_list\":[$openid_list],\"to_groupid\":128}";
				$url = "https://api.weixin.qq.com/cgi-bin/groups/members/batchupdate?access_token=$access_token";
				$output = $this->https_request_qzgdwl($url,$data);
	...
	private function https_request_qzgdwl($url, $data = null) {
		$curl = curl_init();

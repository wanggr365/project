...
class YytAction extends Action {
	...
	private function isWx(){		
		if(!$_SESSION['unionid'] || !$_SESSION['openid'] || !$_SESSION['state']){
			$tv = $_GET['tv'];
			if($tv){
				$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx1ae00a2623048bf1&redirect_uri=http%3a%2f%2fwww.968816.com.cn%2fqzgdwl%2findex.php%3fm%3dlogin%26a%3dindex&response_type=code&scope=snsapi_base&state=jfs%26sjJfs%26$tv#wechat_redirect";
			}else{
				$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx1ae00a2623048bf1&redirect_uri=http%3a%2f%2fwww.968816.com.cn%2fqzgdwl%2findex.php%3fm%3dlogin%26a%3dindex&response_type=code&scope=snsapi_base&state=jfs%26sjJfs#wechat_redirect";
			}
			header("Location: $url");
		}
	}
	...
	public function sjJfs(){
		 ...
		 $this->display(C('HOME_DEFAULT_THEME').':qzgdwlMenuTV');
	...
	public function sjBinding(){
		...
		if(!file_exists('QR/'.$shareCode.".png")){
			...
			$value = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx1ae00a2623048bf1&redirect_uri=http%3a%2f%2fwww.968816.com.cn%2fqzgdwl%2findex.php%3fm%3dshareBinding%26a%3dindex&response_type=code&scope=snsapi_base&state=sj%26sjShareBinding%26$unionid%26$shareCode#wechat_redirect";//二维码内容 
	...
	public function sjBindingYwy(){
		...
		for($i=0;$i<2000;$i++){
			...
			$value = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx1ae00a2623048bf1&redirect_uri=http%3a%2f%2fwww.968816.com.cn%2fqzgdwl%2findex.php%3fm%3dshareBinding%26a%3dindexYwy&response_type=code&scope=snsapi_base&state=sj%26sjShareBinding%26$shareCode#wechat_redirect";//二维码内容 
			$errorCorrectionLevel = 'L';//容错级别 
			$matrixPointSize = 6;//生成图片大小 
			//生成二维码图片 
			QRcode::png($value, 'qrcode.png', $errorCorrectionLevel, $matrixPointSize, 2); 
	...
	public function jfsAjax(){
		...
		if(!$unionid){
			...
			if($jsfRow){				
				...
				$arrReturn = array();
				$arrReturn = $this->runByPraise('预存赠送');	
			...
			if($jfsRow){
				...
				if($values[0]['attributes']['RETURN-CODE'] == 0) {	
					foreach($values as $value1){
						if(isset($value1['attributes']['MERCHANTID']) && $value1['attributes']['MERCHANTID'] == '910002' && floatval($value1['attributes']['AMOUNT']) >= 200 && $value1['attributes']['PAYTIME'] >= $wxTime && !in_array($value1['attributes']['APPPAYNO'],$arrJfs)) {
				...
				if($values[0]['attributes']['RETURN-CODE'] == 0) {					
					foreach($values as $value1){
						if(isset($value1['attributes']['MERCHANTID']) && $value1['attributes']['MERCHANTID'] == '910002' && floatval($value1['attributes']['AMOUNT']) >= 200 && $value1['attributes']['PAYTIME'] >= $wxTime) {
							$jfsRow1 = M('Jfs')->where(array('payid'=>$value1['attributes']['APPPAYNO']))->select();
							if($jfsRow1){
								continue;
							}else{							
								$_SESSION['APPPAYNO'] = $value1['attributes']['APPPAYNO'];
	...
	public function hbqShare(){
		...
		if(!isset($_SESSION['APPPAYNO'])){
			...
			// $shareRow = $share->where(array('payid'=>$_SESSION['APPPAYNO']))->select();
			// if($shareRow){
				// $unionidArr = array();
				// $unionidArr['unionid'] = $unionid;
				// $unionidArr['create_date'] = date('Y-m-d H:i:s',time());
			...
			// }else{
				// $unionidArr = array();
				// $unionidArr['unionid'] = $unionid;
				// $unionidArr['payid'] = $_SESSION['APPPAYNO'];
				// $unionidArr['create_date'] = date('Y-m-d H:i:s',time());
				// $share->data($unionidArr)->add();
				// $arrReturn = array();
				// $arrReturn = $this->runByPraise('分享赠送');
				// $result['code'] = 1;
				// $result['value'] = $arrReturn[0];
			// }
			$unionidArr = array();
			$unionidArr['unionid'] = $unionid;
			$unionidArr['create_date'] = date('Y-m-d H:i:s',time());
